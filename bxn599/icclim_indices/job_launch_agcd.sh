...
indices_bi="tasmax:tasmin:DTR tasmax:tasmin:ETR tasmax:tasmin:vDTR" ##tas:pr:CD tas:pr:CW tas:pr:WD tas:pr:WW" ## AGCD doesn't have tas
...
indices_percentile="tasmax:TX90p tasmax:TX10p tasmin:TN90p tasmin:TN10p pr:R75p pr:R75pTOT pr:R95p pr:R95pTOT pr:R99p pr:R99pTOT"

index_list_all="${indices_tasmax} ${indices_tasmin} ${indices_prcp}" ## don't include ${indices_bi} in list due to running a separate script with longer walltime. don't include ${indices_tas} due to agcd not having tas
