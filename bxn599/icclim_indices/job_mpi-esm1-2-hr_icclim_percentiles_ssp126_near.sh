...
IN_ROOT_DIR=/g/data/xv83/users/bxn599/ACS/data/mpi-esm1-2-hr/ssp126
DOMAIN=GLOBAL-gn
...
# bivariate
  if [ "$count" == "2" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		touch fail.mpi-esm1-2-hr.ssp126.near.${index}
	else
		touch success.mpi-esm1-2-hr.ssp126.near.${index}
	fi
done
done