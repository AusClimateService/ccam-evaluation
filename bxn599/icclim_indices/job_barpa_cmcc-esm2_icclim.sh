...
IN_ROOT_DIR=/g/data/ia39/australian-climate-service/test-data/CORDEX-CMIP6/output/AUS-15/BOM/CMCC-CMCC-ESM2/historical/r1i1p1f1/BOM-BARPA-R/v1/day
DOMAIN=AUS-15
...
# bivariate
  if [ "$count" == "2" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		touch fail.barpa.cmcc-esm2.${index}
	else
		touch success.barpa.cmcc-esm2.${index}
	fi
done
done