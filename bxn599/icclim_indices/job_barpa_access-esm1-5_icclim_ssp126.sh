...
GCM_MODEL_NAME=CSIRO-BOM-ACCESS-ESM1-5
EXPERIMENT_NAME=ssp126
ENSEMBLE_MEMBER=r6i1p1f1
IN_ROOT_DIR=/g/data/ia39/australian-climate-service/test-data/CORDEX-CMIP6/output/AUS-15/BOM/CSIRO-BOM-ACCESS-ESM1-5/ssp126/r6i1p1f1/BOM-BARPA-R/v1/day
DOMAIN=AUS-15
...
# bivariate
  if [ "$count" == "2" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		touch fail.barpa.access-esm1-5.ssp126.${index}
	else
		touch success.barpa.access-esm1-5.ssp126.${index}
	fi
done
done