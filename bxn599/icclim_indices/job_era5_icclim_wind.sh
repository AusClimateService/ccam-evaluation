...
IN_ROOT_DIR=/g/data/xv83/users/bxn599/ACS/data/era5/raw
DOMAIN=AUS-25
...
# bivariate
  if [ "$count" == "2" ]; then
    ...
    for var_name1 in ${var_list1}; do
      ...
      if [ "$var_name1" == "sfcWind" ]; then
                    var_name1=si10
                    TIME_AGG1="mean"
            fi
      if [ "$var_name1" == "sfcWindmax" ]; then
                    var_name1=fg10
                    TIME_AGG1="max --hshift"
            fi
    ...
    for var_name2 in ${var_list2}; do
      ...
      if [ "$var_name2" == "sfcWind" ]; then
                    var_name2=si10
                    TIME_AGG2="mean"
            fi
      if [ "$var_name2" == "sfcWindmax" ]; then
                    var_name2=fg10
                    TIME_AGG2="max --hshift"
            fi
