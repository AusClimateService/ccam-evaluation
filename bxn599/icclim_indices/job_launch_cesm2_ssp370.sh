...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp370_near.sh
done
...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp370_mid.sh
done
...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp370_far.sh
done