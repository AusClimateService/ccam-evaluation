...
indices_bi="tas:pr:CD tas:pr:CW tas:pr:WD tas:pr:WW"
...
index_list_all="${indices_tasmax} ${indices_tasmin} ${indices_prcp} ${indices_tas}" ## don't include ${indices_bi} in list due to running a separate script with longer walltime
...
for index in $index_list_bi; do
		...
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_bivariate_temp.sh
done
