...
indices_tas="tas:TG"
indices_percentile="tas:TG90p tas:TG10p pr:R75p pr:R75pTOT pr:R95p pr:R95pTOT pr:R99p pr:R99pTOT tas:pr:CD tas:pr:CW tas:pr:WD tas:pr:WW pr:SPI3 pr:SPI6 tasmax:TX90p tasmax:TX10p tasmin:TN90p tasmin:TN10p"
...
for index in $index_list_all; do
		...
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_ssp126.sh
done
...
for index in $index_list_per; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_percentiles_ssp126_near.sh
done
...
for index in $index_list_per; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_percentiles_ssp126_mid.sh
done
...
for index in $index_list_per; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_percentiles_ssp126_far.sh
done
...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp126_near.sh
done
...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp126_mid.sh
done
...
for index in $index_list_spi; do
		...
		jobname=index.${data}.${index_list//":"/"_"}
		echo "Submit $jobname"
		qsub -N $jobname -v index_list job_${data}_icclim_spi_ssp126_far.sh
done