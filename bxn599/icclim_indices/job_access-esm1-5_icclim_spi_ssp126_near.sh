...
IN_ROOT_DIR=/g/data/xv83/users/bxn599/ACS/data/access-esm1-5/ssp126
DOMAIN=GLOBAL-gn
...
# bivariate
  if [ "$count" == "2" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		touch fail.access-esm1-5.ssp126.near.${index}
	else
		touch success.access-esm1-5.ssp126.near.${index}
	fi
done
done