...
indices_wind="sfcWind:FG sfcWind:FGcalm sfcWind:FG6Bft sfcWindmax:FXx"
...
for index in $index_list_wind; do
   export index_list=$index
   echo $index_list
   jobname=index.${data}.${index_list//":"/"_"}
   echo "Submit $jobname"
   qsub -N $jobname -v index_list job_${data}_icclim_wind.sh
done