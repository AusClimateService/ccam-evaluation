...
def get_name(indicator):
    if type(indicator) == str:
        return indicator.split("(")[0].split(".")[-1]
    elif 'identifier' in dir(indicator):
...
def xclim_for_agcd(indicator, var, agg, years, outpath=None, outname=None):
    ...
    indicator: xclim indicator: e.g. xclim.indicators.icclim.RX1day
            or string: e.g., "xclim.indices.tx_days_above({arg}, thresh='30 degC', freq='MS')"
                   where {arg} will be substituted with the xarray.dataset
    ...
    with reader(filepaths) as DS:
        ...
        if var == 'precip':
            ...
            ds = ds.assign_attrs(units='degC')

        if type(indicator) == str:
            new = eval(indicator.format(arg='ds'))
        else:
