...
#_DATA_ROOTDIR = '/g/data/oi10/replicas/CMIP6'
...
def get_cmip6_files(gcm, scenario, freq, var, trange=(None, None), rootdir=None, ens=None):
    ...
    assert freq in ['15min', '1hr' , '3hr', '6hr', 'day', 'Amon','fx'], "Unknown freq"
    ...
    files.sort()
    if freq != 'fx': #fixed files (e.g. orography, masks) have no time range
...
def get_cmip6(gcm, scenario, freq, var, 
    ...
    if freq !='fx':
        ...
        if 'gregorian' in cal or 'standard' in cal:
            ...
            tstart = cftime.DatetimeNoLeap(1900, 1, 1) if trange[0] is None else cftime.DatetimeNoLeap(trange[0].year, trange[0].month, trange[0].day, trange[0].hour)
            tend = cftime.DatetimeNoLeap(2200, 1, 1) if trange[0] is None else cftime.DatetimeNoLeap(trange[1].year, trange[1].month, trange[1].day, trange[1].hour)
    ...
    if as_iris:
        ...
        if freq == 'fx':
            out = ds.sel(lat=slice(latmin, latmax), lon=slice(lonmin, lonmax))
        else:
