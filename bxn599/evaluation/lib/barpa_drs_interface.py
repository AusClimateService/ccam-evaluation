...
def get_barpa(gcm, scenario, freq, var, 
    ...
    if 'gregorian' in cal:
        ...
        tstart = cftime.DatetimeAllLeap(1900, 1, 1) if trange[0] is None else cftime.DatetimeNoLeap(trange[0].year, trange[0].month, trange[0].day, trange[0].hour)
        tend = cftime.DatetimeAllLeap(2200, 1, 1) if trange[0] is None else cftime.DatetimeNoLeap(trange[1].year, trange[1].month, trange[1].day, trange[1].hour)
